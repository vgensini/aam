...
def get_mon_aam(year):
	...
	aam_arr = np.zeros((len(dates),361))
	for i,dt in enumerate(dates):
		...
		print dt
	gr.close()
	#aam_arr=np.array(aam_arr)
	#aam_arr.dump(str(year)+month+'_aam.npy')
        #write to netcdf file
	nc = Dataset(str(year)+month+'_aam.nc','w',format='NETCDF4_CLASSIC')
	...
	nc.createDimension('time',len(dates))
        ...
        latitude = nc.createVariable('lats','d',('lat',))
        aamvar = nc.createVariable('aam','d',('time','lat'))
 
        #assign values
        latitude[:] = lats
        aamvar[:] =  aam_arr
        timevar[:] = date2num(dates,units="hours since 1800-1-1 00:00:0.0")
	timevar.units="hours since 1800-1-1 00:00:0.0"
        #close file
	nc.close()
